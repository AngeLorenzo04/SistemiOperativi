...
int main(int argc, char* argv[]) {
    ...
    char Cx,token,max;                   /* variabile che conterrà un singolo carattere */
         ...
         /* codice eseguito dal processo figlio*/
            ...
            while(read(fd,&linea[j],1)){
                ...
                if(linea[j] == '\n'){
                    ...
                    if((write(pipeFiglioPadre[n][1],&linea[j-1],1)) < 0){ /* Scrivo */
                       printf("ERRORE: qualcosa è andato storto nella scrittura 110\n");
                       exit(-1);
                    }
    ...
    while (read(pipeFiglioPadre[0][0],&Cx,1) == 1){
        max=Cx;
        j=0;
        for(n=1;n<N;n++){
            if((read(pipeFiglioPadre[n][0],&Cx,1)) < 0){ /* Leggo */
               printf("ERRORE: qualcosa è andato storto nella letttura ");
               exit(-1);
            }
            if(Cx > max){
