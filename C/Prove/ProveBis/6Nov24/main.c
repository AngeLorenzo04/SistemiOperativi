...
int main(int argc, char* argv[]) {
    ...
    /*Creazione del file */
    if ((fd=creat("/tmp/RISULTATO", PERM)) < 0){
       printf("DEBUG-FILGIO Impossibile creare il file %s\n", "/tmp/RISULTATO");
       exit(-1);
    }

    if((write(fd,argv[1],strlen(argv[1]))) < 0){ /* Scrivo */
       printf("ERRORE: qualcosa è andato storto nella scrittura 48\n");
       exit(-1);
    }

    if((write(fd,"\n",1)) < 0){ /* Scrivo */
       printf("ERRORE: qualcosa è andato storto nella scrittura 53\n");
       exit(-1);
    }
         ...
         /* codice eseguito dal processo figlio*/
            ...
            if (pid == 0) {
             /* codice eseguito dal processo figlio*/
                ...
                /*NON SI DOVREBBE MAI RITORNARE QUI*/
               exit(-1);
            }
            ...
            if ((status & 0xFF) != 0){ //verificare che lo status sia valido2
               printf("Nipote terminato in modo anomalo\n");
                exit(-1);
            }else { //estraggo il valore passato dal nipote
                ritorno = status >> 8;
            ...
            j=0;
            while (read(pipeFiglioNipote[0],&buffer[j],1))
            {
                j++;
            }
            j++;
            ...
            if((write(fd,buffer,j)) < 0){ /* Scrivo */
               printf("ERRORE: qualcosa è andato storto nella scrittura 130\n");
               exit(-1);
            }
            

            if((write(fd,"\n",1)) < 0){ /* Scrivo */
               printf("ERRORE: qualcosa è andato storto nella scrittura 136\n");
               exit(-1);
            }  

            if((write(pipePadreFiglio[n][1],buffer,j)) < 0){ /* Scrivo */
               printf("ERRORE: qualcosa è andato storto nella scrittura 141\n");
               exit(-1);
            }
    ...
    printf("I FILGI HANNO MANDATO:\n\n");
    for(n=0;n<N;n++){

        j=0;
        while (read(pipePadreFiglio[n][0],&buffer[j],1))
        {
            j++;
        }
        j++;
