...
int main(int argc, char* argv[]) {
    ...
    char ch,token;             /* variabile che conterrà un singolo carattere */
    ...
    for(q=0;q<Q;q++){
        ...
        if (pid == 0) {
         /* codice eseguito dal processo figlio*/
            ...
            for(int k=0;k<Q;k++){
               if(k != q) close(pipeRing[k][0]); /* Chiusura dei canali di scrittura che non ci interessano */
            ...
            while (read(fd,&ch,1) == 1){
                ...
                if(i == q+k*Q){ /* Condizione per verificare di aver trovato il nostro target */

                    if((read(pipeRing[q][0],&token,1)) < 0){ /* Aspetto il via libera */
                       printf("ERRORE: qualcosa è andato storto nella letttura 84\n");
                       exit(-1);
                    }

                    printf("Figlio con indice %d e pid %d ha letto il carattere %c\n",q,getpid(),ch);

                    if((write(pipeRing[(q+1)%Q][1],&token,1)) < 0){ /* Dò il via libera */
    ...
    for(q=0;q<Q;q++){ /* Chiudo le pipe inutilizzate */
    ...
    if((write(pipeRing[0][1],&token,1)) < 0){ /* Faccio partire la pipeRing */
       printf("ERRORE: qualcosa è andato storto nella scrittura \n");
       exit(-1);
    }
