...
int main(int argc, char* argv[]) {
    ...
    char token,min,Cx;                     /* variabile che conterrà un singolo carattere */
    ...
    int n,i,i_min,stampa;                          /* Contatore */
    ...
    for(n=0;n<N;n++){
        ...
        if (pid == 0) {
         /* codice eseguito dal processo figlio*/
          ...
          buffer[i] = '\0';
          if(write(pipeFiglioPadre[n][1],&buffer[i-1],1) != 1){
            
            printf("Errore nella scrittura 114\n");
            exit(-1);

          }


          if(read(pipePadreFiglio[n][0],&token,1) != 1){

            printf("Errore nella scrittura  121\n");
            exit(-1);

          }

          if(token == 's'){
            printf("FIGLIO-%d STAMPA LUNGA DAL TESTO\n",n);
            stampa=1;
   ...
   if(read(pipeFiglioPadre[0][0],&min,1) != 1){
     
        printf("Errore nella lettura 151\n");
        exit(-1);
   }
   ...
   for(n=1;n<N;n++){
       ...
       if(read(pipeFiglioPadre[n][0],&Cx,1) != 1){ 
           printf("Errore nella lettura 160\n");
           exit(-1);
       }
       if(Cx < min){
           min = Cx;
   ...
   for(n=0; n<N; n++){
        ...
        if(n==i_min){
            token='s';
            if(write(pipePadreFiglio[n][1],&token,1) != 1){
                printf("Errore nella scrittura 177\n");
                exit(1);
            }
